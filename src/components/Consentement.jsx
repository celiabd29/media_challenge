  ...
  useEffect(() => {
    const fetchAll = async () => {
      const [a, v, p] = await Promise.all([
        supabase.from('articles').select('*').eq('category', 'Consentement'),
        supabase.from('videos').select('*').eq('category', 'Consentement'),
        supabase.from('podcasts').select('*').eq('category', 'Consentement'),
      ]);
  ...
  return (
    <div className="flex flex-col min-h-screen pb-20 bg-white px-4 py-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="ml-2 text-2xl font-medium">Consentement</h1>
      </div>
