  ...
  useEffect(() => {
    const fetchAll = async () => {
      const [a, v, p] = await Promise.all([
        supabase.from('articles').select('*').eq('category', 'Plaisir'),
        supabase.from('videos').select('*').eq('category', 'Plaisir'),
        supabase.from('podcasts').select('*').eq('category', 'Plaisir'),
      ]);
  ...
  return (
    <div className="flex flex-col min-h-screen pb-20 bg-white px-4 py-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="ml-2 text-2xl font-medium">Plaisir</h1>
      </div>
          ...
          <Link href={item.path} key={item.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition flex flex-row sm:flex-col">
            ...
            <div className="p-4 flex flex-col justify-between gap-2 flex-1">
              <div className="flex flex-col gap-1">
                <span className="text-[10px] px-2 py-0.5 bg-pink-100 text-pink-700 rounded-full w-fit text-xs">{item.category || 'Plaisir'}</span>
