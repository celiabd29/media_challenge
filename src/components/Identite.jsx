  ...
  const tabs = [
    {
      key: 'Tout',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 25 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const [a, v, p] = await Promise.all([
        supabase.from('articles').select('*').eq('category', 'Identité'),
        supabase.from('videos').select('*').eq('category', 'Identité'),
        supabase.from('podcasts').select('*').eq('category', 'Identité'),
      ]);
  ...
  return (
    <div className={`flex flex-col min-h-screen pb-20 px-4 py-6 ${darkMode ? "bg-gray-900 text-white" : "bg-white text-gray-800"}`}>
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className={`ml-2 text-2xl font-medium ${darkMode ? "text-white" : "text-gray-800"}`}>Identité</h1>
      </div>
