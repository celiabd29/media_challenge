...
export default function ProtectionPage() {
  ...
  const tabs = [
    ...
    { key: 'Article', icon: (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><line x1="10" y1="9" x2="8" y2="9" /></svg>) },
    { key: 'Vidéo', icon: (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><rect x="2" y="6" width="15" height="12" rx="2" ry="2" /><polygon points="10 8 16 12 10 16" /></svg>) },
    { key: 'Podcast', icon: (<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z" /><path d="M19 10v1a7 7 0 0 1-14 0v-1" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" /></svg>) },
  ];

  useEffect(() => {
    const fetchAll = async () => {
      const [a, v, p] = await Promise.all([
        supabase.from('articles').select('*').eq('category', 'Protection'),
        supabase.from('videos').select('*').eq('category', 'Protection'),
        supabase.from('podcasts').select('*').eq('category', 'Protection'),
      ]);
  ...
  useEffect(() => {
    if (selectedTab === 'Tout') setFiltered(contents);
    else setFiltered(contents.filter((item) => item.type === selectedTab));
  ...
  return (
    <div className="flex flex-col min-h-screen pb-20 bg-white px-4 py-6">
      <div className="flex items-center gap-2 mb-6">
        ...
        <h1 className="ml-2 text-2xl font-medium">Protection</h1>
      </div>
          ...
          <Link href={item.path} key={item.id} className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition flex flex-row sm:flex-col">
            ...
            <div className="p-4 flex flex-col justify-between gap-2 flex-1">
              <div className="flex flex-col gap-1">
                <span className="text-[10px] px-2 py-0.5 bg-pink-100 text-pink-700 rounded-full w-fit text-xs">{item.category || 'Protection'}</span>
