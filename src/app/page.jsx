...
return (
  <main className={`min-h-screen px-2 md:px-8 lg:px-16 pb-20 w-full ${darkMode ? 'bg-[#0F172A] text-white' : 'bg-white text-gray-800'}`}>
    <header className="pt-8 flex items-center justify-between">
    <p className={`text-lg md:text-2xl ${darkMode ? 'text-white' : 'text-gray-700'}`}>
    ...
    <h1 className={`text-xl md:text-3xl font-semibold mb-4 ${darkMode ? 'text-white' : 'text-black'}`}>
  Notre dernier reportage
</h1>
      ...
      <div className="flex gap-x-3 md:gap-x-6 overflow-x-scroll hide-scrollbar">
        {categories.map(cat => (
          <Link key={cat.key} href={cat.href} className="flex-shrink-0 flex flex-col items-center">
            ...
            <p className={`text-sm md:text-lg mb-4 text-center ${darkMode ? 'text-white' : 'text-gray-700'}`}>
  {cat.label}
</p>
